...
export async function fetchYamlContent(webURL: string) {
  const response = await globalThis.fetch(webURL, {
    headers: { 'Content-Type': 'text/yaml' },
    mode: 'cors',
  });
  const openapiDocument = yaml.load(await response.text());
...
export async function getDereferencedOpenAPIDocument(params: OpenAPISpecParams) {
  ...
  const isNodeEnvDetected = globalThis.process?.versions?.node;
  ...
  try {
    if (params.location === 'web') {
      ...
      const { getYamlContent } = await import('./load-yaml.ts');
    ...
    const SwaggerParser = await import('@apidevtools/json-schema-ref-parser');
    const dereferenced = await SwaggerParser.default.dereference(openapiDocument);
    return dereferenced;
  } catch (error: any) {
